...
public class SWEA_5634_키순서_D4 {
	...
	public static void main(String[] args) throws NumberFormatException, IOException {
		...
		for (int tc = 1; tc <= T; tc++) {
			...
			answer=0;
			for (int i = 1; i <= N; i++) {
				if(taller(i,new boolean[N+1]) + shorter(i,new boolean[N+1]) == N-1) ++answer;
			}
	...
	private static int taller(int n, boolean[] visited) {
		int result=0;
	...
	// 자기 자신보다 작은 사람이 몇명인지 리턴하는 함수
	private static int shorter(int n, boolean[] visited) {
		int result=0;
		Queue<Integer>q = new LinkedList<>();
		q.offer(n);
		while(!q.isEmpty()) {
